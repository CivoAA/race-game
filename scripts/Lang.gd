extends Node
## Zentrale Text-/Sprachtabelle. AKTUELL nur die Shop-Hinweise (Tooltips, die über dem
## Upgrade-Namen erscheinen, wenn man ~1 s darüber hovert).
##
## Weitere Sprachen hinzufügen: pro Sprachcode einen weiteren Block in HINTS anlegen
## (gleiche Upgrade-IDs als Schlüssel) und set_language("en"/"fr"/…) aufrufen. Fehlt für
## die gewählte Sprache ein Text, wird auf Deutsch ("de") zurückgegriffen.
##
## Platzhalter: {value} im Text wird NICHT hier, sondern vom Aufrufer (UpgradeMenu)
## durch den aktuellen Upgrade-Wert ersetzt – so bleibt der Text sprachunabhängig.
##
## HINWEIS / TODO: Sobald wir eine externe Datei `res://Lang.json` (o. ä.) anlegen, soll dieses
## Skript durch sie ERSETZT werden – die Texte kommen dann aus der JSON-Datei statt aus der
## HINTS-Konstante hier. Bis dahin sind die Texte bewusst im Skript hinterlegt. Die öffentliche
## API (hint(), set_language(), get_language()) sollte beim Umstieg gleich bleiben, damit die
## Aufrufer (UpgradeMenu) unverändert weiterlaufen.

const FALLBACK_LANG := "de"

# Aktuelle Sprache. Wird als Autoload gehalten, damit eine spätere Sprachumschaltung
# (Einstellungen) global wirkt, ohne dass die einzelnen Menüs etwas davon wissen müssen.
var _lang := "de"

# id → Hinweistext, gruppiert nach Sprachcode. Nur Upgrades, die im Upgrade-Menü
# ("Allgemeines"/"Bonusfelder") auftauchen, brauchen hier einen Eintrag.
const HINTS := {
	"de": {
		"speed":
...
# Sprache umschalten (nur wirksam, wenn für den Code Texte hinterlegt sind).
...
## Hinweistext für ein Upgrade in der aktuellen Sprache. Leerer String, wenn weder in der
## aktuellen noch in der Fallback-Sprache ein Text hinterlegt ist (→ Aufrufer zeigt keinen Tooltip).
func hint(id: String) -> String:
	var table: Dictionary = HINTS.get(_lang, {})
	if table.has(id):
		return String(table[id])
	return String(HINTS.get(FALLBACK_LANG, {}).get(id, ""))
